function launchFileExplorer() {
  if (!window.os || !window.os.win || typeof window.os.win.openWindow !== 'function') {
    console.error('File Explorer: window.os.win.openWindow is not available.');
    return;
  }
  ...
  console.log(`File Explorer launched in window ${windowId}`);
  return windowId; // Return windowId for potential external management
}

// Make launchFileExplorer globally accessible
if (!window.apps) {
  window.apps = {};
}
window.apps.launchFileExplorer = launchFileExplorer;

// Example of how to immediately launch it for testing (optional)
// if (window.os && window.os.win) {
//   launchFileExplorer();
// } else {
//   // If desktop isn't fully loaded, wait for a bit or for an event
//   console.warn("File Explorer: OS components not ready for immediate launch. Consider launching via Start Menu or event.");
// }
